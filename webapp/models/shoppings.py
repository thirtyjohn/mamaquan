...
def getindexsps():
    return dbconn.query("""
    (select * from shopping where itemclass='nvzhuang' and picked=1 order by score desc limit 6)
    union
    (select * from shopping where itemclass='nvxie' and picked=1 order by score desc  limit 6)
    union
    (select * from shopping where itemclass='danjianbao' and picked=1  order by score  desc  limit 6)    
    """)
